	...
	rmln.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `*BEM VINDO(A) ${mdata.subject}*
___________________________
${a}@${num.split('@')[0]} Aprensente-se orangutango!${a}
${a}➸ Nome :${a}
${a}➸ Idade :${a}
${a}➸ Sexo? :${a}
${a}➸ Gênero :${a}
...
${a}Não seja uma pulga!${a}`
				...
				teks = `ADEUS KKK @${num.split('@')[0]}👋*
${a}Vou enterrar seu corpo profundamente${a}`
				let buffer = await getBuffer(ppimg)
				rmln.sendMessage(mdata.id, buffer, MessageType.image, { caption: teks, contextInfo: { "mentionedJid": [num] } })
			}
		} catch (e) {
	...
	rmln.on('message-new', async (Lan) => {
        ...
        if (Rank <= 2) {
			...
			if (isGroup) {
				...
				setTimeout(() => {
					rmln.groupRemove(from, [kic]).catch((e) => { reply(`BOCA PORCA`) })
				}, 3000)
				setTimeout(() => {
					rmln.updatePresence(from, Presence.composing)
					reply("O JOGO")
			...
			if (budy.includes("https://chat.whatsapp.com/")) {
				...
				setTimeout(() => {
					rmln.groupRemove(from, [kic]).catch((e) => { reply(`EU TENHO QUE SER ADM GARAI`) })
				}, 3000)
			}
...
━━ 「 *GRR BOT* 」 ━━`
				...
				case 'senpai':
					...
					rmln.sendMessage(from, 'Esse é o meu senpaii>_<(número dele)', MessageType.text, { quoted: Lan })
					break
...
${a}❏ Nome : ${rmln.user.name}${a}
${a}❏ Browser : ${rmln.browserDescription[1]}${a}
${a}❏ Server : ${rmln.browserDescription[0]}${a}
${a}❏ Versão : ${rmln.browserDescription[2]}${a}
${a}❏ Velocidade: ${latensii.toFixed(4)} Second${a}
${a}❏ Celular : ${rmln.user.phone.device_manufacturer}${a}
${a}❏ Versão do zap : ${rmln.user.phone.wa_version}${a}
     ...
     case 'stickerwa':
				...
				reply(nad.wait())
                    ...
                    for (var x of get_result) {
                        ...
                        rmln.sendMessage(from, ini_buffer, sticker)
                    }
                    break
                    case 'wait':
                if (!isRegistered) return reply( nad.noregis())
				...
				reply(nad.wait())
                    if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Lan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : Lan
                        const filePath = await rmln.downloadAndSaveMediaMessage(encmedia, filename = getRandom());
                        ...
                        rmln.sendMessage(from, get_video, video, { mimetype: 'video/mp4', filename: `${get_info.title}.mp4`, quoted: Lan })
                    } else {
				...
				reply(nad.wait())
					...
					result = fs.readFileSync(`stctagg.webp`)
                                        rmln.sendMessage(from, result, sticker, selepbot)
                    ...
                    if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
                    if (!isPrem) return reply(nad.premium(prefix))
                        const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(Lan).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : lol
                        filePath = await rmln.downloadAndSaveMediaMessage(encmedia, filename = getRandom())
                        var value = args.join(" ")
                        var group = await rmln.groupMetadata(from)
                        var member = group['participants']
                        var mem = []
                        ...
                        rmln.sendMessage(from, ini_buffer, image, options)
                        fs.unlinkSync(filePath)
                    } else {
				...
				case 'nuliskiri':
					...
					rmln.sendMessage(from, kir, image, { quoted: Lan, caption: 'Aqui, mano' })
					break
				case 'nuliskanan':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
					...
					rmln.sendMessage(from, kan, image, { quoted: Lan, caption: 'Aqui, mano' })
					break
				case 'stalkig':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
...
◯ Biografia : ${abu.biography}`
				...
				case 'tts':
					...
					if (isLimit(sender)) return reply(nad.limitend(pusname, prefix))
					await limitAdd(sender)
					if (args.length < 1) return rmln.sendMessage(from, `Cadê o código de idioma? mande !idiomas pra saber quais são`, text, { quoted: Lan })
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return rmln.sendMessage(from, `Cadê o texto porra | Exemplo : ${prefix}tts id ah yamate kudasai`, text, { quoted: Lan })
				...
				case 'bikinquote':
					...
					rmln.sendMessage(from, biquote, image, { caption: 'Aqui >_<', quoted: Lan })
					break
				case 'groupmenu':
				case 'grupomenu':
					if (isBanned) return reply(nad.baned())
...
「 *${botName}* 」`
				...
				case 'antipalavrao':
					...
					if (Number(args[0]) === 1) {
						...
						reply('──『✙ FEITO ✙』──O grupo é educacional agora🙏🏻')
						rmln.sendMessage(from, `ALLERT!!! Nada de palavrão no grupo hein!\nSe você falar, eu vou te banir`, text)
					} else if (Number(args[0]) === 0) {
						if (!isBadWord) return reply('Já Morto')
						var ini = antilink.indexOf(from)
						badword.splice(ini, 1)
						fs.writeFileSync('./database/badword.json', JSON.stringify(badword))
						reply('──『✙ FEITO ✙』──O GRUPO NÃO É MAIS EDUCACIONAL PORRA CARALHO BUCETA FILHO DA PUTAAAAAAAAKKKKKKKK🤡🤡👉👌')
					} else {
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						reply('──『✙ FEITO ✙』──Recurso de boas-vindas ativado')
					} else if (Number(args[0]) === 0) {
						if (!isWelkom) return reply('Ja feito')
						welkom.splice(from, 1)
						fs.writeFileSync('./database/welkom.json', JSON.stringify(welkom))
						reply('──『✙ FEITO ✙』──Recurso de boas-vindas desativado')
					} else {
						reply('1 para ligar, 0 para desligar')
					}
					break
				...
				case 'leveling':
					...
					if (Number(args[0]) === 1) {
						...
						reply('──『✙ FEITO ✙』──Recurso de nível ativado')
					} else if (Number(args[0]) === 0) {
						_leveling.splice(from, 1)
						fs.writeFileSync('./database/leveling.json', JSON.stringify(_leveling))
						reply('──『✙ FEITO ✙』──Recurso de nível desativado')
					} else {
						reply('1 para ligar, 0 para desligar')
					}
					break
				...
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						reply('──『✙ FEITO ✙』──QUEM VIR FLOODAR NOSSO GRUPO VAI TER O CU COMIDO(e é sem cuspe)')
						rmln.sendMessage(from, `ALLERTA!!! Este grupo está protegido pela companhia GRR-SECURITY KAKAK\nQuem mandar link vai pra vala!`, text)
					} else if (Number(args[0]) === 0) {
						if (!isAntiLink) return reply('Ja morto')
						var ini = antilink.indexOf(from)
						antilink.splice(ini, 1)
						fs.writeFileSync('./database/antilink.json', JSON.stringify(antilink))
						reply('──『✙ FEITO ✙』──Podem divulgar link')
					} else {
						reply('1 para ligar, 0 para desligar')
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (Lan.message.extendedTextMessage === undefined || Lan.message.extendedTextMessage === null) return reply('Marque o desgraçado')
				...
				case 'setname':
					...
					rmln.sendMessage(from, '──『✙ FEITO ✙』──Nome do grupo mudado', text, { quoted: Lan })
					break
				...
				case 'setdesc':
					...
					rmln.sendMessage(from, '*──『✙ FEITO ✙』──Descrição do grupo mudada', text, { quoted: Lan })
					break
				...
				case 'demot':
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (Lan.message.extendedTextMessage === undefined || Lan.message.extendedTextMessage === null) return reply('Marque o alvo que você deseja chutar!')
				...
				case 'promot':
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (Lan.message.extendedTextMessage === undefined || Lan.message.extendedTextMessage === null) return reply('Marque o bem aventurado')
					mentioned = Lan.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						prom = ''
						for (let _ of mentioned) {
	    ...
	    if (!isOwner) return reply(nad.ownerb())
				...
				case 'hs':
					...
					if (!isGroupAdmins) return reply(nad.admin())
					if (!isBotGroupAdmins) return reply(nad.badmin())
					if (Lan.message.extendedTextMessage === undefined || Lan.message.extendedTextMessage === null) return reply('Marque o Alvo hehe')
					mentioned = Lan.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
...
「 *${botName}* 」`
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, ini_buffer, image, { quoted: Lan, caption: ini_txt })
                    get_audio = await getBuffer(get_result.audio[3].link)
                    rmln.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_info.title}.mp3`, quoted: Lan })
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, ini_buffer, image, { quoted: Lan, caption: get_result.title })
                    get_audio = await getBuffer(get_result.audio)
                    rmln.sendMessage(from, get_audio, audio, { mimetype: Mimetype.mp4Audio, filename: `${get_result.title}.mp3`, quoted: Lan })
                    get_video = await getBuffer(get_result.video)
                    rmln.sendMessage(from, get_video, video, { mimetype: Mimetype.mp4, filename: `${get_result.title}.mp4`, quoted: Lan })
                    break
                case 'ytsearch':
                if (!isRegistered) return reply( nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: Lan })
                    break
                case 'ytmp32':
                case 'baixarmp32':
                if (!isRegistered) return reply( nad.noregis())
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: Lan })
                    break
                case 'ytmp42':
                case 'baixarmp42':
                if (!isRegistered) return reply( nad.noregis())
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, thumbnail, image, { quoted: Lan, caption: ini_txt })
                    get_audio = await getBuffer(get_result.link[3].link)
                    rmln.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.title}.mp3`, quoted: Lan })
                    break
                case 'spotifysearch':
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, ini_buffer, ini_type, { quoted: Lan })
                    break
                case 'fbdl':
                if (!isRegistered) return reply( nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				...
				reply(nad.wait())
                    ...
                    ini_buffer = await getBuffer(ini_url)
                    rmln.sendMessage(from, ini_buffer, video, { quoted: Lan })
                    break
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, get_audio, audio, { mimetype: 'audio/mp4', filename: `${get_result.info.song}.mp3`, quoted: Lan })
                    break
...
「 *${botName}* 」`
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
                    ...
                    rmln.sendMessage(from, buffer, image, {caption: 'Aqui mano.. *Divirta-se Y:V*', quoted: Lan})
                break
	...
	if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
	  ted = isQuotedImage ? JSON.parse(JSON.stringify(rmln).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo: Lan
	  reply(nad.wait())
	  owgi = await rmln.downloadAndsavemediamessage(encmedia)
	  tels = body.slice(7)
	  anu = await imgbb("3ea1465ef91578a90ee81f7d41c59a1f", owgi)
	  hehe = await getBuffer(`https://lolhuman.herokuapp.com/api/editor/alien?apikey=${lolhumankey}&img=${anu.display_url}`)
	 rmln.sendMessage(from, hehe, image, {quoted: Lan})
	} else {
...
if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 rmln.sendMessage(from, hedhe, image, {quoted:Lan})
} else {
...
var imgbb = require('imgbb-uploader')
if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
  ger = isQuotedImage ? JSON.parse(JSON.stringify(rmln).replace('quotedM', 'm')).message.extendedTextMessage.contextinfo: Lan
  reply(nad.wait())
  owgi = await rmln.downloadAndsavemediamessage(encmedia)
  anu = await imgbb("08579d070df9a07cb1c2ee565aece767", owgi)
...
rmln.sendMessage(from, nobg, sticker, {
  quoted: Lan
})
fs.unlinkSync(rano)
  })
...
if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 rmln.sendMessage(from, hehpe, image, {quoted:Lan})
} else {
  reply('Marque a imagem burro!')
}
break
...
if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
 ...
 rmln.sendMessage(from, dhehe, image, {quoted:Lan})
} else {
  reply('Marque a imagem, burro!!')
}
break
...
case 'carta':
         if (!isRegistered) return reply(nad.noregis())
				...
				rmln.sendMessage(`${noorg2}@s.whatsapp.net`, `╔════ ◤ *CARTA VIRTUAL* ◢
║╔▸
...
╚═══ ◤ *${botName}* ◢ `, text, {quoted: { key: {fromMe:false, participant:`${noorg2}@s.whatsapp.net`},
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta: *'+gayy+'*\n\nA viadagem dele é nível🏳️‍🌈: '+ yag+'%', text, { quoted: Lan })
					await limitAdd(sender)
					break
                    case 'quantolesbica':
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta: *'+lesbicek+'*\n\nOlha o quanto lésbica ela é👩‍❤️‍💋‍👩: '+ bi+'%', text, { quoted: Lan })
					await limitAdd(sender)
					break
	...
	if ((isMedia && !Lan.message.videoMessage || isQuotedImage) && args.length == 0) {
	 ...
	 rmln.sendMessage(from, gender, text, {quoted:Lan})
	} else {
	  reply('Não adicione nada ao comando')
	}
	break
...
break
				...
				case 'acho':
					...
					setTimeout(() => {
						rmln.sendMessage(from, '*➸ Responda :* ' + anu.result.jawaban, text, { quoted: Lan })
					}, 30000) // 1000 = 1s,
					setTimeout(() => {
						rmln.sendMessage(from, '_10 Outro segundo…_', text) // ur cods
					}, 20000) // 1000 = 1s,
					setTimeout(() => {
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta : *'+bisakah+'*\n\nResposta: '+ keh, text, { quoted: Lan })
					await limitAdd(sender)
					break
		            case 'quando':
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta : *'+kapankah+'*\n\nResposta: '+ koh, text, { quoted: Lan })
					await limitAdd(sender)
					break
		            case 'pergunta':
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta : *'+apakah+'*\n\nResposta: '+ kah, text, { quoted: Lan })
					await limitAdd(sender)
					break
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta : *'+rate+'*\n\nA nota é: '+ te+'%', text, { quoted: Lan })
					await limitAdd(sender)
					break
...
Exemplo : ${somtoy4}<=====`, text, { quoted: Lan })
            break
    ...
    case 'personalidade':
				...
				if (isBanned) return reply('Desculpe, você foi banido pelo senpaii! Por tanto não pode usar os comandos da Grr. !')
					...
					rmln.sendMessage(from, 'Pergunta : *'+watak+'*\n\nEssa pessoa tem uma personalidade: '+ tak, text, { quoted: Lan })
					await limitAdd(sender)
				        break
				...
				case 'hobby':
					...
					rmln.sendMessage(from, 'Pergunta: *'+hobby+'*\n\nEssa pessoa costuma: '+ by, text, { quoted: Lan })
					break
...
「 *${botName}* 」`
                ...
                if (!isNsfw) return reply(nad.nsfwoff())
                 ...
                 reply(nad.wait())
                 asww = anjglu.teks
                 rmln.sendMessage(from, dapbokepp, image, {quoted: Lan, caption: asww})
                 break
		...
		if (!isRegistered) return reply(nad.noregis())
				...
				rmln.sendMessage(from, buffer, image, {quoted: Lan, caption: 'Aqui, seu safado...'})
				break
		        case 'nekonimee': 
		if (!isNsfw) return reply(nad.nsfwoff())
		if (!isRegistered) return reply(nad.noregis())
		...
		if (!isRegistered) return reply(nad.noregis())
				...
				buffer = await getBuffer(anu.result.url)
				rmln.sendMessage(from, buffer, image, {quoted: Lan, caption: 'Abaxuuu(つ≧▽≦)つ'})
				break
		        case 'hug2': 
		if (!isNsfw) return reply(nad.nsfwoff())
		if (!isRegistered) return reply(nad.noregis())
				...
				buffer = await getBuffer(anu.result.url)
				rmln.sendMessage(from, buffer, image, {quoted: Lan, caption: 'Abaxuoo'})
					break
				...
				reply(nad.wait())
                    ...
                    rmln.sendMessage(from, ini_buffer, image, { quoted: Lan })
                    break
                case 'bj':
                case 'ero':
                case 'cum':
		        ...
		        if (!isRegistered) return reply(nad.noregis())
                    ...
                    exec(`wget ${ini_buffer} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
                        fs.unlinkSync(ranp)
                        buff = fs.readFileSync(rano)
                        rmln.sendMessage(from, buff, sticker, { quoted: Lan})
                        fs.unlinkSync(rano)
                    })
                    break
...
「 *${botName}* 」`
				...
				case 'gachacewek':
					...
					sendImage(hasil, Lan, 'Então, que tal Bwang?:v')
					break
				...
				case 'gachacowok':
					...
					randKey = jsonData[randIndex];
					hasil = await getBuffer(randKey.result)
					sendImage(hasil, Lan, 'E quanto a Mba?:v')
					break
				case 'meme':
					if (isBanned) return reply(nad.baned())
					if (!isRegistered) return reply(nad.noregis())
    ...
    if (!isOwner) return reply(nad.ownerb())
                   ...
                   if (!isNsfw) return reply(nad.nsfwoff())
					...
					rmln.sendMessage(from, buffer, sticker, { mimetype: 'sticker/webp', quoted: Lan })
					break
					...
					await rmln.sendMessage(from, ini_buffer, sticker, { quoted: Lan })
    ...
    if (!isOwner) return reply(nad.ownerb())
                   ...
                   if (!isNsfw) return reply(nad.nsfwoff())
					...
					rmln.sendMessage(from, buffer, gif, video,  { mimetype: 'video/mp4', quoted: Lan })
					break
...
「 *${botName}* 」`
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						rmln.sendMessage(from, buffer, image, { quoted: Lan, caption: 'aqui [(^.^)]' })
						fs.unlinkSync(ran)
					})
					break
...
➸ URL : ${data.display_url}`
				...
				case 'komenyt':
					...
					rmln.sendMessage(from, buffer, image, { caption: 'Nih Cok', contextInfo: { participant: '0@s.whatsapp.net', quotedMessage: { conversation: '*_COMENTÁRIO DO YOUTUBE_*' } } })
					break
...
if (isGroup) return  reply( 'Este recurso não pode ser executado em grupos')
				...
				try {
				ppimg = await rmln.getProfilePicture(`${anug.split('@')[0]}@c.us`)
				} catch {
				ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
				}
  teks = `‣ *Nick* : ${pushname}
...
Clique em APi para iniciar o chat`
  ...
  rmln.sendMessage(from, Ciecie, image, {
quoted: Lan, caption: teks
  })
...
try {
ppimg = await rmln.getProfilePicture(`${hem.split('@')[0]}@c.us`)
  } catch {
ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
  }
  teks = `‣ *Nick* : ${pushname}
...
Clique em APi para iniciar o chat`
  ...
  rmln.sendMessage(from, its, image, {
quoted: Lan, caption: teks
  })
...
「 *${botName}* 」`
				...
				case 'addsticker':
					...
					if (!stiklan) return reply('Agora nomeie o sticker!')
				...
				case 'addvideo':
					...
					rmln.sendMessage(from, `Pronto, video adicionado ao meu banco de dados✧◝(⁰▿⁰)◜✧`, MessageType.text, { quoted: Lan })
					break
				...
				case 'addvn':
					...
					rmln.sendMessage(from, `Pronto, áudio adicionado ao meu banco de dados✧◝(⁰▿⁰)◜✧`, MessageType.text, { quoted: Lan })
					break
				...
				case 'addimage':
					...
					rmln.sendMessage(from, `Pronto, imagem adicionado ao meu banco de dados✧◝(⁰▿⁰)◜✧`, MessageType.text, { quoted: Lan })
					break
...
「 *${botName}* 」`
				...
				case 'bc':
					...
					if (isMedia && !Lan.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							rmln.sendMessage(_.jid, buff, image, { caption: `*「 ${botName} AVISO 」*\n\n${body.slice(4)}` })
						}
						reply('')
					} else {
						for (let _ of anu) {
				...
				case 'setprefix':
					...
					if (!isOwner) return reply(nad.ownerb())
					prefix = args[0]
					fakestatus(`*──『✙ FEITO ✙』──* Prefixo é ➸ : ${prefix}`)
					break
				...
				case 'event':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*──『✙ FEITO ✙』──ATIVEI OS EVENTOS EM GRUPO*')
					} else if (Number(args[0]) === 0) {
						event.splice(from, 1)
						fs.writeFileSync('./database/event.json', JSON.stringify(event))
						reply('*──『✙ FEITO ✙』──EVENTOS DESLIGADOS EM GRUPO*')
					} else {
						reply('Selecione 1/0')
					}
					break
...
╰─────「 *${botName}* 」`
                ...
                rmln.sendMessage(from, options, text, {quoted: Lan, contextInfo: { forwardingScore: 1000, isForwarded: true }})
                  }
			if (budy == 'cekprefix') {
		...
		if (budy == 'Bot on?') {
				reply(`Mãe tá On com certeza, mande !menu (ou !comandos) pra ver os comandos ;)`)
			}
			if (budy == 'Te amo grr') {
