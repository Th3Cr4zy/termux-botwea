...
async function starts() {
	...
	rmln.on('credentials-updated', () => {
		...
		info('2', 'Faça login no coração dela')
	})
	...
	rmln.on('open', () => {
		success('2', 'Prontinho, tô on de novo :D')
	})
	...
	const mdata = await rmln.groupMetadata(anu.jid)
			if (anu.action == 'add'){
				...
				if(!num.split('@')[0].startsWith(55)) {
					rmln.sendMessage(mdata.id, ' Numeros estrangeiros ou fakes não sao Permitidos neste grupo, 🏌️', MessageType.text)
					setTimeout(async function () {
						rmln.groupRemove(mdata.id, [num])
					}, 1000)
			    }
			}
		}
...
「 *${botName}* 」`
				...
				case 'setname':
					...
					rmln.sendMessage(from, '──『✙ FEITO ✙』──\nNome do grupo mudado', text, { quoted: Lan })
					break
				...
				case 'setdesc':
					...
					rmln.sendMessage(from, '*──『✙ FEITO ✙』──\nDescrição do grupo mudada', text, { quoted: Lan })
					break
...
buffer = fs.readFileSync(`./media/video/fis.mp4`)
			...
			if (isGroup && !isCmd && budy != undefined) {
				...
				console.log(color('[404]', 'red'), 'Comando desconhecido de', color(sender.split('@')[0]))
			}
  } catch (e) {
